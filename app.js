    ...
    binders.forEach((binder, index) => {
        ...
        div.innerHTML = `
                    ...
                    <div class="binder-actions">
                        ...
                        <button
                            class="binder-action edit-binder"
                            onclick="event.stopPropagation(); editBinder(${index})">
                            ✎
                        </button>

                        <button
                            class="binder-action delete-binder"
                            onclick="event.stopPropagation(); deleteBinder(${index})">
                            🗑
                        </button>
...
function openBinderModal() {
    ...
    document.querySelector("#binderDialog h2").textContent =
        "Create Binder";
...
function editBinder(index) {
    ...
    document.getElementById("binderColor").value =
        binder.color || "#222222";
...
function openBinder(index) {
    ...
    document.getElementById("binderTitle").innerText =
        binders[index].name;
...
function openCardModal(cardIndex = null) {
    ...
    document.getElementById("cardDialogTitle").innerText =
        cardIndex === null ? "Add Card" : "Edit Card";
...
function saveCard() {
    ...
    if (file) {
        ...
        reader.onload = function () {
            ...
            finishSave(reader.result, name, set, number);
...
function finishSave(image, name, set, number) {
    ...
    const card = {
        ...
        set,
        number,
        qty: editingCard !== null
            ? binders[currentBinder].cards[editingCard].qty
            : 0
    };
    ...
    binders[currentBinder].cards.forEach((card, index) => {
        ...
        div.innerHTML = `
                    ...
                    : ''}
                            ...
                            <button class="edit"
                                onclick="openCardModal(${index})">✎</button>
...
function toggleOrder() {
    ...
    document.querySelector(".floating").style.display =
        orderMode ? "none" : "block";

    renderCards();

    document.getElementById("cardsGrid")
        .classList.toggle("ordering", orderMode);
}
